...
[params.binary_image_processing,remain] = strtok(Config.Threshold.Filter,'+');
[~,background] = strtok(remain,'_'); 
params.background_filter=str2double(background(2:end));
...
if Config.FirstTFrame>0
    ...
    if JobNr>0
        ...
        h=progressdlg('String',sprintf('Tracking - Frame: %d',Config.FirstTFrame),'Min',Config.FirstTFrame-1,'Max',min([Config.LastFrame length(Stack)]),'Cancel','on','Parent',hMainGui.fig);
