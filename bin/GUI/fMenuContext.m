...
if ~isempty(kObj)
    if strcmp(Mode{1},'Molecule')
        Molecule=fShared('AddDataMol',Object,Objects,kObj,kData,hMainGui.Values.FrameIdx,n);
        Molecule(kObj).Results(:,5) = fDis(Molecule(kObj).Results(:,3:4));
    else
        Filament=fShared('AddDataFil',Object,Objects,kObj,kData,hMainGui.Values.FrameIdx,n);
