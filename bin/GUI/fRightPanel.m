...
line(hMainGui.Scan.InterpD*hMainGui.Values.PixSize/1000,I,'Color','blue','LineStyle','-','UIContextMenu',hMainGui.Menu.ctScan,'Tag','plotLineScan');
...
KymoGraph = zeros(length(Stack),length(d));    
N = length(Stack);
hMainGui=getappdata(0,'hMainGui'); 
progressdlg('String','Creating KymoGraph','Min',0,'Max',length(Stack),'Parent',hMainGui.fig);
if get(hMainGui.RightPanel.pTools.cKymoDrift,'Value')==1 && ~isempty(Drift)
    if get(hMainGui.RightPanel.pTools.mKymoMethod,'Value')==1
        for n = 1:N
            [~,m]=min(abs(Drift(:,1)-n));
            Z = interp2(double(Stack{n}),iX+Drift(m,2)/hMainGui.Values.PixSize,iY+Drift(m,3)/hMainGui.Values.PixSize,'nearest');
            KymoGraph(n,:)=max(Z,[],1);
            progressdlg(n);
        end
    else
        for n = 1:N
            [~,m]=min(abs(Drift(:,1)-n));
            Z = interp2(double(Stack{n}),iX+Drift(m,2)/hMainGui.Values.PixSize,iY+Drift(m,3)/hMainGui.Values.PixSize);
            KymoGraph(n,:)=mean(Z,1);
            progressdlg(n);
        end 
    ...
    if get(hMainGui.RightPanel.pTools.mKymoMethod,'Value')==1
        for n = 1:N
            Z = interp2(double(Stack{n}),iX,iY,'nearest');
            KymoGraph(n,:)=max(Z,[],1);
            progressdlg(n);
        end
    else
        for n = 1:N
            Z = interp2(double(Stack{n}),iX,iY);
            KymoGraph(n,:)=mean(Z,1);
            progressdlg(n);
        end 
    end
