...
if ~isempty(Stack)
    ...
    elseif strcmp(get(hMainGui.ToolBar.ToolThreshImage,'State'),'on')
        [filter,remain] = strtok(Config.Threshold.Filter,'+'); 
        [~,background] = strtok(remain,'_'); 
        params = struct('scale',Config.PixSize,'fwhm_estimate',Config.Threshold.FWHM/Config.PixSize,'binary_image_processing',filter,'background_filter',str2double(background(2:end)));
        if strcmp(get(hMainGui.ToolBar.ToolRedGreenImage,'State'),'off')
