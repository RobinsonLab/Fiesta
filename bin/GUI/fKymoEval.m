...
if ~isempty(Stack)
    ...
    if ~isempty(hKymoEval.ref) && ~isempty(mode)
        ...
        hKymoEval.hImage = image(Image,'Parent',hKymoEval.aImage,'CDataMapping','scaled','EraseMode','normal');
        set(hKymoEval.aImage,'CLim',[0 65535],'YDir','reverse','NextPlot','add','TickDir','in','Visible','off'); 
...
T = (StackInfo.CreationTime(Y)-StackInfo.CreationTime(Y(1)))*sign(StackInfo.CreationTime(Y(end))-StackInfo.CreationTime(Y(1)))/1000;
...
if isempty(Data)
    ...
    set(hKymoEval.lResults,'Data',Data(:,1:4),'Enable','on','ColumnFormat',{'numeric','bank','bank','bank'},'ColumnWidth',{small,big,big,big},'RowName','','ColumnName',{'Idx','<html><CENTER>Distance<br><CENTER>[µm]','<html><CENTER>Time<br><CENTER>[s]','<html><CENTER>Velocity<br><CENTER>[µm/s]'});
...
KymoPix = mean(d);
KymoGraph = zeros(length(Stack),scan_length);           
for i = 1:length(Stack)
    Z = interp2(double(Stack{i}),iX,iY);
    KymoGraph(i,:)=max(Z,[],1);
end
...
elseif FilterIndex==2
    ...
    f = fopen(file,'w');
    fprintf(f,'Index\tDistance[µm]\tTime[s]\tVelocity[µm/s]\n');
    fprintf(f,'%f\t%f\t%f\t%f\n',Data(:,1:4)');
    fclose(f);
end
...
fid = fopen(f,'w');
fprintf(fid,'Index\tDistance[µm]\tTime[s]\tVelocity[µm/s]\n');
fprintf(fid,'%f\t%f\t%f\t%f\n',Data(:,1:4)');
fclose(fid);
Close([],[]);
